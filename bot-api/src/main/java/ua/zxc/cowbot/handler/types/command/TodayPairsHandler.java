...
@Handler(value = "TODAY_PAIRS", commands = "/today")
@Slf4j
@RequiredArgsConstructor
public class TodayPairsHandler implements HandlerStrategy {
    ...
    public SendMessage handleMessage(Update update) {
        ...
        try {
            messageText = processFindingTodayPairsForChat(chatId);
        ...
        return createMessageForTodayPairs(chatId, messageText);
    }

    private String processFindingTodayPairsForChat(Long chatId) throws PairNotFoundException, JSONObjectConvertException {
        String groupName = getGroupNameByChatId(chatId);
        String todayPairs = scheduleService.getTodayPairs(groupName);
        if (todayPairs.isBlank()) {
    ...
    private SendMessage createMessageForTodayPairs(Long chatId, String messageText) {
