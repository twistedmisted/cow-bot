...
@Handler(value = "PAY_RESPECT")
@Slf4j
public class PayRespectHandler implements HandlerStrategy {

    private final RespectService respectService;

    private final UserService userService;

    public PayRespectHandler(RespectService respectService, UserService userService) {
    ...
    public SendMessage handleMessage(Update update) {
        ...
        try {
            return payRespectToUserAndCreateMessageToSend(message);
        } catch (RuntimeException e) {
            log.warn("Cannot pay respect to user, something went wrong", e);
            return SendMessage.builder()
                    .chatId(message.getChatId())
                    .text(String.format(TRY_AGAIN_WHAT_SOMETHING, "кинути вповажайку"))
                    .build();
        }
    }

    private SendMessage payRespectToUserAndCreateMessageToSend(Message message) {
        Message replyToMessage = message.getReplyToMessage();
        if (isNull(replyToMessage) || isRepliedHimself(message) || isFromBot(replyToMessage)) {
            return new SendMessage();
        }
        payRespectToUser(message);
    ...
    private void payRespectToUser(Message message) {
        User userToRespect = message.getReplyToMessage().getFrom();
        if (isUserRegistered(userToRespect.getId())) {
            respectService.payRespect(createRespectDTOFromMessage(message));
        }
    }
